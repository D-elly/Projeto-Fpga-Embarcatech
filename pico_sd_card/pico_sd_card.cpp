...
int main()
{
    ...
    if (!cartao.montarSistemaArquivos()) {
        ...
        if (wav_file.estaAberto()) {
            ...
            // WAVE é encontrado em Big Endian como 0x45 0x56 0x41 0x57, 
            // mas o FF library pode estar lendo em Little Endian dependendo da configuração.
            // Vamos verificar o 'WAVE' de forma robusta:
            if (header_buffer[12] == 'W' && header_buffer[13] == 'A' && 
                header_buffer[14] == 'V' && header_buffer[15] == 'E') {
                
                printf("VERIFICACAO 2 (WAVE): OK\n");

                // O formato PCM deve ser 1 (Little Endian)
                uint16_t audio_format = (uint16_t)(header_buffer[34] | (header_buffer[35] << 8));
                uint16_t num_channels = (uint16_t)(header_buffer[38] | (header_buffer[39] << 8));
                uint32_t sample_rate = (uint32_t)(header_buffer[40] | (header_buffer[41] << 8) | 
                                                (header_buffer[42] << 16) | (header_buffer[43] << 24));
